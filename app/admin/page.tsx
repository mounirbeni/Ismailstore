...
export default function AdminPage() {
  ...
  async function handleToggleAvailability(id: string, available: boolean) {
    ...
    try {
      ...
      setAdminMenuItems(prev => prev.map(i => i.id === id ? { ...i, available: !available } : i));
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex gap-1 bg-gray-100 p-1 rounded-xl">
          ...
          <button
            onClick={() => { setActiveTab('menu'); if (adminMenuItems.length === 0) loadMenu(); }}
              ...
              <>
                ...
                <div className="flex items-center justify-between bg-white rounded-2xl px-5 py-3 shadow-sm border border-gray-100 text-sm">
                  <span className="text-gray-500">
                    <span className="font-black text-green-600">{adminMenuItems.filter(i => i.available !== false).length}</span>
                    {' '}disponibles
                    {' · '}
                    <span className="font-black text-gray-400">{adminMenuItems.filter(i => i.available === false).length}</span>
                    {' '}masqués
                  </span>
                  <button
                    onClick={loadMenu}
                ...
                {(['tajins', 'salads', 'briwat', 'couscous'] as const).map(cat => {
                  ...
                  const availableCount = catItems.filter(i => i.available !== false).length;
                  return (
                    <div key={cat}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="ml-auto text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
                          {availableCount}/{catItems.length}
                        </span>
                      </div>
                      <div className="space-y-2">
                        {catItems.map(item => {
                          ...
                          return (
                            <div
                              key={item.id}
                              className={`bg-white rounded-xl p-4 flex items-center gap-3 shadow-sm border border-gray-100 transition-opacity duration-200 ${
                                isAvailable ? '' : 'opacity-50'
                              }`}
                            >
                              <div className="flex-1 min-w-0">
                                <p className={`font-semibold text-sm truncate ${isAvailable ? 'text-gray-900' : 'text-gray-400 line-through'}`}>
                                  {item.name}
                                </p>
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className="font-bold text-amber-600 text-xs">{item.price} DH</span>
                                  {item.badge && (
                                    <span className="text-xs text-gray-400 capitalize bg-gray-100 px-1.5 py-0.5 rounded-full">
                                      {item.badge}
                                    </span>
                                  )}
                                </div>
