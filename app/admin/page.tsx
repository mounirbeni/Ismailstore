      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-sm">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <input type="password" value={password} onChange={e => { setPassword(e.target.value); setLoginError(''); }} placeholder="Mot de passe" autoFocus
                className={`w-full px-4 py-3.5 rounded-xl border text-sm outline-none transition-colors ${loginError ? 'border-red-400 bg-red-50' : 'border-gray-200 focus:border-amber-400'}`} />
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={loadOrders} className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-colors" title="Actualiser"><RefreshCw className="w-4 h-4 text-gray-600" /></button>
            <button onClick={() => setAuthed(false)} className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-colors" title="Déconnexion"><LogOut className="w-4 h-4 text-gray-600" /></button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {(['all','new','preparing','on_the_way','delivered','cancelled'] as const).map(f => {
            ...
            const label = f === 'all' ? `Toutes (${orders.length})` : `${STATUS_CONFIG[f].label} (${count})`;
            return (
        ...
        {displayed.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center shadow-sm">
            ...
            <p className="text-gray-400 text-sm mt-1">{filter === 'all' ? 'Les nouvelles commandes apparaîtront ici automatiquement' : 'Aucune commande avec ce statut'}</p>
            {lastRefresh && <p className="text-gray-300 text-xs mt-4">Actualisé à {lastRefresh.toLocaleTimeString('fr-MA')}</p>}
          </div>
        ) : (
          <div className="space-y-4">
            ...
            {displayed.map(order => {
              ...
              return (
                <div key={order.id} className={`bg-white rounded-2xl shadow-sm overflow-hidden border-l-4 ${cfg.border} ${order.status === 'new' ? 'ring-2 ring-orange-200' : ''}`}>
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="font-black text-gray-900 text-lg">{order.orderNumber}</h3>
                          <span className={`text-xs px-2.5 py-1 rounded-full font-bold ${cfg.bgColor} ${cfg.textColor}`}>{cfg.label}</span>
                        </div>
                        <div className="flex items-center gap-1.5 mt-1 text-gray-400 text-xs">
                          <Clock className="w-3 h-3" /><span>{timeAgo(order.createdAt)}</span>
                        </div>
                      </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3 mb-4 space-y-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        <a href={`tel:${order.customer.phone}`} className="flex items-center gap-1 text-xs text-amber-600 hover:text-amber-700">
                    ...
                    <div className="space-y-1.5 mb-4">
                      {order.items.map(item => (
                        <div key={item.id} className="flex items-center gap-2 text-sm">
                          ...
                          <span className="font-semibold text-gray-900 w-16 text-right">{item.price * item.quantity} DH</span>
                        </div>
                      ))}
                    ...
                    {order.status !== 'delivered' && order.status !== 'cancelled' && (
                      <div className="flex gap-2">
                        {nextAction && (
                          <button onClick={() => handleStatusUpdate(order.id, nextAction.status)}
                            className={`flex-1 py-3 text-white rounded-xl font-bold text-sm transition-colors ${nextAction.cls}`}>{nextAction.label}</button>
                        )}
                        <button onClick={() => handleStatusUpdate(order.id, 'cancelled')}
                          className="px-4 py-3 bg-red-50 text-red-500 rounded-xl font-bold text-sm hover:bg-red-100 transition-colors">Annuler</button>
                      </div>
                    )}
                  </div>
                </div>
