...
import { X, ChevronRight, ChevronLeft, MapPin, User, Phone, CheckCircle, Clock, MessageCircle, Banknote } from 'lucide-react';
...
export default function CheckoutModal({ isOpen, onClose }: Props) {
  ...
  const [customer, setCustomer] = useState<CustomerInfo>({ name: '', phone: '', neighborhood: '', address: '', notes: '' });
  ...
  function validateStep1() {
    ...
    else if (!/^(\+?212|0)[5-7][0-9]{8}$/.test(customer.phone.replace(/[\s-]/g, ''))) e.phone = 'Numéro invalide (ex: 06XXXXXXXX)';
    setErrors(e);
        ...
        <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col max-h-[95vh]">
            ...
            {step === 1 && (
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <input type="text" value={customer.name} onChange={e => setCustomer(c => ({...c, name: e.target.value}))} placeholder="Ex: Mohammed Alami"
                ...
                <div>
                  ...
                  <input type="tel" value={customer.phone} onChange={e => setCustomer(c => ({...c, phone: e.target.value}))} placeholder="06XXXXXXXX ou 07XXXXXXXX"
            ...
            {step === 2 && (
              <div className="space-y-5">
                <div>
                  ...
                  <select value={customer.neighborhood} onChange={e => setCustomer(c => ({...c, neighborhood: e.target.value}))}
                ...
                <div>
                  ...
                  <textarea value={customer.address} onChange={e => setCustomer(c => ({...c, address: e.target.value}))} placeholder="Rue, numéro, bâtiment, étage..." rows={3}
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Instructions <span className="text-gray-400 font-normal">(optionnel)</span></label>
                  <textarea value={customer.notes} onChange={e => setCustomer(c => ({...c, notes: e.target.value}))} placeholder="Code d'accès, sonnez à la porte..." rows={2}
            ...
            {step === 4 && placedOrder && (
              <div className="flex flex-col items-center text-center py-4 gap-5">
                ...
                <div className="flex items-center gap-2 text-gray-600 bg-gray-50 rounded-2xl px-5 py-3 w-full">
                  ...
                  <span className="text-sm">Livraison estimée: <strong>30–45 minutes</strong></span>
                </div>
                ...
                <a href={buildWhatsAppUrl(placedOrder)} target="_blank" rel="noopener noreferrer"
                  ...
                  <MessageCircle className="w-5 h-5" />Suivre via WhatsApp
                </a>
                <button onClick={handleClose} className="text-amber-600 font-semibold text-sm hover:text-amber-700 transition-colors">← Retourner au menu</button>
              </div>
            )}
          </div>
          ...
          {step <= 3 && (
            <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0">
              {step < 3 ? (
                <button onClick={handleNext} className="w-full py-4 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2 transition-colors">
                  Continuer <ChevronRight className="w-4 h-4" />
                </button>
              ) : (
                <button onClick={handleConfirmOrder} className="w-full py-4 bg-green-500 hover:bg-green-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2 transition-colors shadow-lg shadow-green-200">
                  <CheckCircle className="w-5 h-5" />Confirmer la commande · {total} DH
                </button>
              )}
            </div>
          )}
