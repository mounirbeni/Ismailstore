    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-black text-gray-900 leading-none">Suivi de commande</h1>
            <p className="text-gray-400 text-xs">Dar Ismail · Marrakech</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Ex: DI-142537"
              className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-amber-400 transition-colors font-mono tracking-widest"
            />
            <button
              onClick={() => search()}
        ...
        {notFound && !order && (
          <div className="bg-white rounded-2xl p-8 text-center shadow-sm border border-gray-100">
            ...
            <p className="font-semibold text-gray-900">Commande introuvable</p>
            <p className="text-gray-400 text-sm mt-1">Vérifiez votre numéro et réessayez</p>
          </div>
        )}
        ...
        {order?.status === 'cancelled' && (
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-red-100">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-black text-gray-900 text-lg">Commande annulée</p>
                <p className="text-red-500 font-mono font-bold">{order.orderNumber}</p>
                <p className="text-gray-400 text-xs mt-0.5">Cette commande a été annulée</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="font-black text-amber-600 text-2xl font-mono">{order.orderNumber}</p>
                </div>
                    ...
                    <div key={step.status} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-500 ${
                          isDone   ? 'bg-green-500' :
                          isActive ? 'bg-amber-500 ring-4 ring-amber-100' :
                                     'bg-gray-100'
                        }`}>
                          <Icon className={`w-5 h-5 ${isDone || isActive ? 'text-white' : 'text-gray-400'}`} />
                        </div>
                        {!isLast && (
                          <div className={`w-0.5 h-8 mt-1 rounded-full transition-all duration-500 ${isDone ? 'bg-green-400' : 'bg-gray-200'}`} />
                        )}
                      </div>
              ...
              {order.status === 'delivered' && (
                <div className="mt-5 bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-green-800 text-sm">Commande livrée !</p>
                    <p className="text-green-600 text-xs">Merci pour votre confiance. Bon appétit !</p>
                  </div>
                </div>
              )}
            </div>
        ...
        {!searched && (
          <div className="bg-white rounded-2xl p-10 text-center shadow-sm border border-gray-100">
            ...
            <p className="font-bold text-gray-900 text-lg">Suivez votre commande</p>
            <p className="text-gray-400 text-sm mt-2">
              Entrez votre numéro de commande<br />pour voir l&apos;état en temps réel
            </p>
          </div>
        )}
      </div>
